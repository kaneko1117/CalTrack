...
| メンバー | ファイル | 役割 | 入力 | 出力 |
|---------|---------|------|------|------|
| プランナー | `planner.md` | 設計 | 機能要件 | 設計ドキュメント |
| エンジニア | `engineer.md` | 実装 | 設計ドキュメント | 実装ファイル一覧 |
| QA | `qa.md` | テスト | 実装ファイル一覧 | テスト結果 |
| DevOps | `devops.md` | リリース | テスト結果 | PR URL |
| 技術リード | `tech-lead.md` | リファクタ | 対象コード | 修正ファイル一覧 |
...
```
PM → プランナー → [設計承認] → 子Issue作成 → エンジニア → QA → [テスト承認] → DevOps
                      ↑                                         ↑
                忘れずに作成！                              忘れずに確認！
```
...
| ポイント | タイミング | 確認内容 |
|---------|-----------|---------|
| **設計承認** | プランナー完了後 | 設計内容をユーザーに確認 |
| **テスト承認** | QA完了後 | PR作成に進んでよいかユーザーに確認 |

**PMは必ず2箇所でユーザー確認を取ること。直接次のメンバーに振らない。**
...
ユーザー → メイン: 「OK」
メイン → PM: 「承認もらいました」
PM → メイン: 「よし、子Issue作成してエンジニアに実装振ろう」
```

### 5. 子Issue作成（重要）

**設計承認後、実装前に必ず子Issueを作成すること。**

```
メイン → DevOps: 「子Issue作成して」
DevOps → メイン: 「Issue #XX 作成しました」
メイン → PM: 「子Issue作成しました」
PM → メイン: 「OK、エンジニアに実装振ろう」
```
